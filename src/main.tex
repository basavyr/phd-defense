...
\begin{frame}
	...
	\end{figure}
	Harris parameters (\textit{Harris, 1965}): $\mathcal{I}_0=30\ \hbar^2\text{MeV}^{-1}$, $\mathcal{I}_1=40\ \hbar^2\text{MeV}^{-3}$ % S. M. Harris, Phys. Rev. 138, B509 (1965
	\begin{beamercolorbox}[rounded=true,shadow=false, wd=\linewidth,]{block body}
		\centering
		\textcolor{red}{\footnotesize{A. A. Raduta, \textbf{R. Poenaru}, C. M. Raduta, Phys. Rev. C 101, 2020.}}
	\end{beamercolorbox}
...
\begin{frame}
	...
	\begin{beamercolorbox}[rounded=true,shadow=false, wd=\linewidth,]{block body}
		\centering
		\textcolor{black}{\small{\textbf{Onset of a redesign}}}\\
...
\begin{frame}
	...
	\begin{exampleblock}{Remarks}
		\begin{itemize}
			...
			\item \faMinus\ $\mathbf{\epsilon_{13/2}^1}$ and $\mathbf{\epsilon_{13/2}^2}$ agree with microscopic calculations % ad-hoc corrections 
		\end{itemize}
	\end{exampleblock}
	\begin{beamercolorbox}[rounded=true,shadow=false, wd=\linewidth,]{block body}
		\centering
...
\begin{frame}
	...
	\begin{exampleblock}{New Hamiltonian}
		...
		\end{align}
		separated Kinetic and Potential terms in the \emph{Schrodinger Equation}:
		\begin{align}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item the motion of the core and the odd nucleon treated separately
			\item realistic results concerning wobbling energies and other quantities
			\item Special attention to the geometrical interpretation of the wobbling motion was given for $^{163}$Lu
		\end{itemize}
		\item The models can be used to \emph{predict} new mass regions where Wobbling Motion could exist
		\item 
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	PhD Program: 2018 - 2023 (PhD Scholarship)
	\begin{itemize}
		\item \textbf{7} ISI papers (2 JPG, 2 PRC, 2 RJP, 1 IJMPE)
		\begin{itemize}
			\item 3 papers as first author
			\item \textbf{39} citations
			\item \textbf{Total IF:} 17.225 ; \textbf{Total AIS:} 4.684
		\end{itemize}
		\item \textbf{8} (\textbf{6} Oral and \textbf{2} Poster) presentations at \textbf{international} conferences
		\item $>5$ seminars/oral presentations at \textbf{national} conferences/events
		\item \textbf{75} total citation
		\item $h$-index: 5
	\end{itemize}
\end{frame}
...
\begin{frame} % The optional argument 'plain' hides the headline and footline
	\begin{center}
		...
		\begin{figure}
			\centering
			\includegraphics[scale=0.45]{figures/cis-platin.pdf}
		\end{figure}
	\end{center}
\end{frame} 
